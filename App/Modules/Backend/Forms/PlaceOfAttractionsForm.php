 ...
 * @author amit
 */
class PlaceOfAttractionsForm extends CForm {
    ...
    public function initialize($entity = null, $options = null) {
        ...
        $attractionName = new Text('attraction_name');
        $attractionName->setLabel('Name of Attraction');
        $attractionName->setAttribute('class', 'form-control');
        if (isset($options['edit']) && $options['edit']) {
            if ($entity && isset($entity->attractionName)) {
                $attractionName->setDefault($entity->attractionName);
            }
        }
        $attractionName->addValidators(array(
            new PresenceOf(array('message' => 'Name of attraction is required'))
        ));
        $this->add($attractionName);

        $status = new Select('attraction_status');
